...
`docs/MULTITENANT_DESIGN.md` §7의 구성 중 **`0001`~`0009`(테넌시 코어 · RBAC · CI 가드 · 프로필/초대 · 역량 체계 · 문제은행 · 시험 도메인 · 감독/녹화 · 인증서)** 가 원격 프로젝트에 적용되어 있습니다.

동작하는 것: 회원가입 → 조직 생성(`/onboarding`) → 초대 발급·수락(`/admin/members`, `/invite/accept`) → 조직 전환.
DB 준비됨(화면 이식 대기): 시험·감독·녹화·인증서 테이블은 모두 원격에 있습니다.
녹화·감독 화면은 스키마가 프런트와 일치하나, **인증서 화면은 등급 어휘 이식이 필요**합니다 —
`certifications` 는 원본의 `grade`(green/blue/black enum) 대신 발급 시점 등급을 동결
(`grade_level_id` + `grade_label`/`grade_code`)하므로, 화면이 아직 옛 `grade` 컬럼을 참조합니다.
